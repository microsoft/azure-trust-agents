...
async def get_agents():
    ...
    # Try API key authentication first, fall back to environment credential
    ai_foundry_key = os.environ.get("AI_FOUNDRY_KEY")
    
    # Create the AI Project Client with proper authentication
    if ai_foundry_key:
        print("🔑 Using AI Foundry API Key authentication...")
        client = AIProjectClient(
            endpoint=endpoint,
            credential=AzureKeyCredential(ai_foundry_key)
        )
    else:
        print("🔑 Using environment credential authentication...")
        # Use environment variables for authentication 
        # This should work with AZURE_CLIENT_ID, AZURE_CLIENT_SECRET, AZURE_TENANT_ID
        os.environ["AZURE_CLIENT_ID"] = ""  # We'll use API keys instead
        os.environ["AZURE_CLIENT_SECRET"] = ""
        os.environ["AZURE_TENANT_ID"] = ""
        
        # For Azure Container Apps, we'll use the endpoint and key directly
        client = AIProjectClient(
            endpoint=endpoint,
            credential=AzureKeyCredential(azure_openai_key)  # Use OpenAI key as fallback
        )
    print("✅ Connected to AI Foundry endpoint.")
    
    try:
        # Create Data Ingestion Agent as ChatCompletionAgent to avoid auth issues
        print(f"🔍 Creating Data Ingestion Agent...")
        
        data_agent = ChatCompletionAgent(
            name="DataIngestionAgent",
            description="Agent that ingests and normalizes transaction and customer data.",
            instructions=(
                """You are a data ingestion specialist for fraud detection analysis.
                
                Your role is to:
                1. Extract and normalize customer data from the database
                2. Retrieve transaction details and history
                3. Format the data in a structured way for analysis
                4. Provide comprehensive customer and transaction context
                
                Always include:
                - Customer ID and basic profile information
                - Transaction details (amount, type, location, timestamp)
                - Recent transaction history for pattern analysis
                - Any relevant customer behavioral indicators
                
                Present your findings in a clear, structured format that other agents can easily analyze.
                """
            ),
        ...
        # Create Transaction Analyst Agent as ChatCompletionAgent
        print(f"⚠️ Creating Transaction Analyst Agent...")
        
        transaction_agent = ChatCompletionAgent(
            name="TransactionAnalystAgent",
            description="Agent that analyzes transactions for fraud risk using ML predictions.",
            instructions=(
                """You are a transaction fraud analyst with expertise in detecting fraudulent patterns.
                
                Your role is to:
                1. Analyze transaction data provided by the Data Ingestion Agent
                2. Use ML predictions and statistical models to assess fraud risk
                3. Identify suspicious patterns and anomalies
                4. Calculate fraud probability scores and risk levels
                5. Pay special attention to transactions near 10,000 EUR threshold as they may indicate structuring
                
                Always provide:
                - Fraud Risk Score (0.0 to 1.0)
                - Risk Level (LOW/MEDIUM/HIGH)
                - Specific risk factors identified
                - Pattern analysis (especially for amount-based patterns)
                - Comparison with customer's normal behavior
                
                Focus on identifying potential structuring attempts where transactions are kept just below regulatory thresholds.
                """
            ),
            service=AzureChatCompletion(
                deployment_name=azure_openai_deployment,
        ...
        # Create a dummy client reference (not actually used now)
        client = None
